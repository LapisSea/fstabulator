...
pub const GENERIC_OPTIONS: &[OptionSpec] = &[
	...
	opt!("x-initrd.mount", OptionValue::Toggle, "Mount the filesystem in the initrd and keep it mounted until final shutdown (systemd)."),
	opt!("x-systemd.after", OptionValue::String, "Mount after the given systemd unit or mount path without starting it."),
	opt!("x-systemd.automount", OptionValue::Toggle, "Mount on first access via an automount unit, ignoring the auto and noauto options (systemd)."),
	opt!("x-systemd.before", OptionValue::String, "Mount before the given systemd unit or mount path without starting it."),
	opt!("x-systemd.device-bound", OptionValue::String, "Bind the mount to its backing device lifetime so it stops when the device disappears, taking yes (also bare) or no."),
	opt!("x-systemd.device-timeout", OptionValue::String, "Limit how long fstab waits for the device to appear, as seconds or a duration such as 30s."),
	opt!("x-systemd.graceful-option", OptionValue::String, "Append a native kernel mount option only when the kernel supports it (systemd 258+)."),
	opt!("x-systemd.growfs", OptionValue::Toggle, "Grow the filesystem to fill its block device when mounted, where supported (systemd)."),
	opt!("x-systemd.idle-timeout", OptionValue::String, "Unmount an idle automount after a duration such as 5min, or pass 0 to disable the timeout."),
	opt!("x-systemd.makefs", OptionValue::Toggle, "Initialize a new filesystem on the device when it carries no existing signatures (systemd)."),
	opt!("x-systemd.mount-timeout", OptionValue::String, "Limit how long the fstab mount command may run, as seconds, a duration such as 30s, or infinity."),
	opt!("x-systemd.pcrfs", OptionValue::Toggle, "Measure the mounted filesystem identity into TPM PCR 15 (systemd 253+)."),
	opt!("x-systemd.required-by", OptionValue::String, "Make a unit require this mount, replacing default dependencies so ordering may need setting explicitly."),
	opt!("x-systemd.requires", OptionValue::String, "Require the given systemd unit, device path, or mount path and mount after it."),
	opt!("x-systemd.requires-mounts-for", OptionValue::String, "Require the mounts needed to reach an absolute path and mount after them."),
	opt!("x-systemd.rw-only", OptionValue::Toggle, "Fail immediately when a read-write mount fails instead of retrying read-only (systemd)."),
	opt!("x-systemd.validatefs", OptionValue::Toggle, "Validate the mounted filesystem constraint metadata after mounting (systemd 258+)."),
	opt!("x-systemd.wanted-by", OptionValue::String, "Make a unit want this mount, replacing default dependencies so ordering may need setting explicitly."),
	opt!("x-systemd.wants", OptionValue::String, "Want the given systemd unit and mount after it without requiring success (systemd 257+)."),
	opt!("x-systemd.wants-mounts-for", OptionValue::String, "Want the mounts needed to reach an absolute path and mount after them without requiring success."),
